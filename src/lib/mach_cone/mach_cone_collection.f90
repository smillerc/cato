 ...
 contains
   ...
   subroutine get_p_prime_quantities(self, recon_operator)
     ...
     real(rk) :: rho, rho_prime
     real(rk) :: p, p_prime
     integer(ik) :: i, j, ni, nj, c, nc
     integer(ik) :: pp_i !< P' i
     integer(ik) :: pp_j !< P' j
     integer(ik) :: n_p_prime_cells
     real(rk) :: pressure_p_prime, density_p_prime
     ...
     do j = 1, nj
       do i = 1, ni
         ...
         self%density_p_prime(i, j) = recon_operator%reconstruct_at_point(i=pp_i, j=pp_j, &
                                                                          ...
                                                                          var='rho')
         !  write(*,'(3(a,es16.6))') "rho(P'): ", self%density_p_prime(i,j), " rho_tilde:", self%reference_density(i,j), &
         !             ' |diff|: ', abs(self%density_p_prime(i,j) - self%reference_density(i,j))
       end do
     end do

     ! FIXME: get P' from the highest pressure cell
     ! Are the P' i,j values right? -> unit test this...
     do j = 1, nj
       do i = 1, ni
         pp_i = self%p_prime_ij(1, i, j)
         pp_j = self%p_prime_ij(2, i, j)
         ...
         write(*, '(a,es16.6, a, 10(es16.6))') "p(P'): ", self%pressure_p_prime(i, j), " -> p:", self%recon_p(:, i, j)
       end do
     end do
     ! error stop
     !  do j = 1, nj
