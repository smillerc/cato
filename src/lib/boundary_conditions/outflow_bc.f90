module mod_outflow_bc
  ...
  private
  public :: outflow_bc_t, outflow_bc_constructor

  type, extends(boundary_condition_t) :: outflow_bc_t
  contains
    procedure, public :: apply_conserved_var_bc => apply_outflow_conserved_var_bc
    procedure, public :: apply_reconstructed_state_bc => apply_outflow_reconstructed_state_bc
    procedure, public :: apply_cell_gradient_bc => apply_outflow_cell_gradient_bc
    procedure, public :: copy => copy_outflow_bc
  end type
contains

  function outflow_bc_constructor(location) result(bc)
    type(outflow_bc_t), pointer :: bc
    character(len=2), intent(in) :: location !< Location (+x, -x, +y, or -y)
    allocate(bc)
    bc%name = 'outflow'
    bc%location = location
  end function outflow_bc_constructor

  subroutine copy_outflow_bc(out_bc, in_bc)
    class(boundary_condition_t), intent(in) :: in_bc
    class(outflow_bc_t), intent(inout) :: out_bc
  end subroutine

  subroutine apply_outflow_conserved_var_bc(self, conserved_vars)
    !< Apply outflow boundary conditions to the conserved state vector field
    class(outflow_bc_t), intent(in) :: self
    ...
    ! case default
    !   error stop "Unsupported location to apply the bc at in outflow_bc_t%apply_outflow_cell_gradient_bc()"
    ! end select

  end subroutine apply_outflow_conserved_var_bc

  subroutine apply_outflow_reconstructed_state_bc(self, reconstructed_state)
    !< Apply outflow boundary conditions to the reconstructed state vector field
    ...
    ! case default
    !   error stop "Unsupported location to apply the bc at in outflow_bc_t%apply_outflow_reconstructed_state_bc()"
    ! end select

  end subroutine apply_outflow_reconstructed_state_bc

  subroutine apply_outflow_cell_gradient_bc(self, cell_gradient)
    !< Apply outflow boundary conditions to the reconstructed state vector field
  ...
  end subroutine apply_outflow_cell_gradient_bc
end module mod_outflow_bc
