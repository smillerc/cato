...
contains
  ...
  function new_pressure_source(input) result(pressure_source)
    ...
    pressure_source%constant_pressure = input%apply_constant_source
    if(.not. pressure_source%constant_pressure) then
      pressure_source%input_filename = trim(input%source_file)
      call pressure_source%read_pressure_input()
    else
      pressure_source%pressure_input = input%constant_source_value
    end if
    ...
    pressure_source%ilo = input%source_ilo
    pressure_source%ihi = input%source_ihi
    pressure_source%jlo = input%source_jlo
    pressure_source%jhi = input%source_jhi
  ...
  subroutine apply_pressure_source(self, primitive_vars, lbounds, time)
    class(pressure_source_t), intent(inout) :: self
    ...
    if(pressure > 0.0_rk) then
      ...
      if(self%ilo == 0 .and. self%ihi == 0) then
        write(*, '(a, es14.5, 2(a, i0), a)') "Applying pressure = ", pressure, " [erg] at (i=:, j=", self%jlo, ":", self%jhi, ")"
        ...
        write(*, '(a, es14.5, 2(a, i0), a)') "Applying pressure = ", pressure, " [erg] at (i=", self%ilo, ":", self%ihi, ", j=:)"
